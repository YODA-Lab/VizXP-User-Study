...
d3.json(window.location.href+'/../explanation_started').then(function(preload_data){
...
boards = [user_svg, agent_svg]
////////////////////////////////////////////////////////////////////

// FOR TEXT

function get_flow_and_states_text(boardindex=0, board){

    d3.json(window.location.href+'/../flow_state/'+ boardindex).then(function(data){
        // board.attr('viewBox', '0 0 ' + (w*2) + ' ' + (Math.max(50 + 50*data[0].length, w)))
        console.log('Solution: ', data[2])
        let action_stack = data[2]

        let flow_data = data[1];
        cur_flow_data[boardindex] = flow_data;
        // board.select('.flow_g').remove();
        let flow_g = boards[boardindex].append('g')
        ...
        cur_flow_data[boardindex].forEach(function(d, i){
            ...
            offset = 5
            board.attr('viewBox', '0 0 ' + (w) + ' ' + (offset+50*cur_flow_data[boardindex].length))
            ...
            board.append('rect').attr('id','viz_plan_step_'+i).attr('class','plan_rect').attr('x',0).attr('y',offset+ 50*i).attr('width',w).attr('height',45)
                .attr('fill',function(){
            ...
            else text= action_stack[i-1][0]+'('+action_stack[i-1].slice(1, action_stack[i-1].length) +')'
            board.append('text')
                .attr("y", offset+25+50*i )
                .attr("x", w/2)
                .attr("dy", "0.1em")
                .attr('font-size','12')
                .attr('fill','#000')
...
//////////////////////////////////////////////////////////////////////////
if(viz_type=='text'){
    get_flow_and_states_text(0, user_svg)
    // get_flow_and_states_text(1, agent_svg)
